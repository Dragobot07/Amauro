...
namespace RDG
{
    ...
    public static class Vibration
    {
        ...
        /// If 'cancel' is true, Cancel() will be called automatically.
        /// </summary>
        public static void Vibrate(long[] pattern, int[] amplitudes = null, int repeat = -1, bool cancel = false, PlayerController playerController = null)
        {
            ...
            if (isInitialized == false)
            {
//                if (playerController != null) { playerController.DebugLog("notInitial"); }
                logAuto(funcToStr() + ": Not initialized", logLevel.Warning);
            }
            else if (HasVibrator() == false)
            {
 //               if (playerController != null) { playerController.DebugLog("no vib"); }
                logAuto(funcToStr() + ": Device doesn't have Vibrator", logLevel.Warning);
            }
            else
            {
                ...
                {
//                    if (playerController != null) { playerController.DebugLog("amp length wrong"); }
                ...
                if (cancel)
                {
//                    if (playerController != null) { playerController.DebugLog("Cancel"); }
                    Cancel();
                }
                if (doesSupportVibrationEffect())
                {
                    ...
                    {
//                        if (playerController != null) { playerController.DebugLog("no amp cont"); }
                        logAuto(funcToStr() + ": Device doesn't have Amplitude Control, but Amplitudes was set", logLevel.Warning);
                        amplitudes = null;
                    }
                    if (amplitudes != null)
                    {
//                        if (playerController != null) { playerController.DebugLog("amp"); }
                    ...
                    {
//                        if (playerController != null) { playerController.DebugLog("nonamp"); }
